...
Y3 = repmat((KukaPlane - 0.08) , size(X3,1), size(Z3,1));
% surf(X3,Y3,Z3);
...
Y5 = repmat((UR5EEPlane+0.08) , size(X5,1), size(Z5,1));
surf(X5,Y5,Z5);
...
Y6 = repmat((KukaEEPlane - 0.08), size(X6,1), size(Z6,1));
surf(X6,Y6,Z6);
...
% UR5 collision Check
    if check5 == 1
        ...
        distanceDif =   (puckCoordinates(1)) - (UR5EE(1));
        disp("Distance to EE is:");
        disp(distanceDif);
        if (-blockRadi < distanceDif) && (distanceDif< blockRadi) % the puck has collided with the blocker. bounce the puck back
            PuckVector = (Puckcenter - Puckcenter_P)/norm(Puckcenter - Puckcenter_P);
...
% Kuka collision Check    
    if check6 == 1
        ...
        distanceDif = (puckCoordinates(1)) - (KukaEE(1));
        disp("Distance to EE is:");
        disp(distanceDif);
        if (-blockRadi < distanceDif) && (distanceDif< blockRadi) % the puck has collided with the blocker. bounce the puck back
